...
public class distributer {
	...
	public static int[] distributer = { 10103, 10203, 10303, 20103, 20203, 20303,
			30103, 30203, 30303, 40103, 40203, 40303, 50103, 50203, 50303 };
		...
		try {
			...
			for (int i = 0; i < distributer.length; i++) {
				...
				while (end_day <= 170701) {
					...
					// count
					sql = "select count(*),sum(price) from staff_transit_day where (distribute1_tid=" + staff_id
							+ " or distribute2_tid=" + staff_id + ") and (start_time between " + start_day * 10000 + " and "
							+ end_day * 10000 + ");";
					...
					sql = "select count(*),sum(price) from staff_distribute_day where (distribute1_tid=" + staff_id
							+ " or distribute2_tid=" + staff_id + ") and (start_time between " + start_day * 10000 + " and "
							+ end_day * 10000 + ");";
					pstmt = (PreparedStatement) conn.prepareStatement(sql);
					rs = pstmt.executeQuery();
		...
		try {
			...
			for (int i = 0; i < distributer.length; i++) {
				...
				while (end_day < 1707) {
					...
					// count
					sql = "select count(*),sum(price) from staff_transit_month where (distribute1_tid=" + staff_id
							+ " or distribute2_tid=" + staff_id + ") and (start_time between " + start_day * 1000000 + " and "
							+ end_day * 1000000 + ");";
					pstmt = (PreparedStatement) conn.prepareStatement(sql);
					rs = pstmt.executeQuery();
					rs.next();
					...
					sql = "select count(*),sum(price) from staff_distribute_month where (distribute1_tid=" + staff_id
							+ " or distribute2_tid=" + staff_id + ") and (start_time between " + start_day * 1000000 + " and "
							+ end_day * 1000000 + ");";
					pstmt = (PreparedStatement) conn.prepareStatement(sql);
					rs = pstmt.executeQuery();
	...
	public static void main(String[] args) {
		distributer here=new distributer();
//		here.getDay();
		here.getMonth();
	}
}
