...
public class transiter {
	...
	public static int[] transit = { 10001, 10002, 20001, 20002,
			30001, 30002, 40001, 40002, 50001, 50002};
		...
		try {
			...
			for (int i = 0; i < transit.length; i++) {
				...
				while (end_day <= 170701) {
					...
					// count
					sql = "select count(*),sum(price) from staff_transit_day where (transit1_tid=" + staff_id
							+ " or transit2_tid=" +staff_id+" ) and (start_time between " + start_day * 10000 + " and "
							+ end_day * 10000 + ");";
		...
		try {
			...
			for (int i = 0; i < transit.length; i++) {
				...
				while (end_day < 1707) {
					...
					// count
					sql = "select count(*),sum(price) from staff_transit_month where (transit1_tid=" + staff_id
							+ " or transit2_tid=" +staff_id+" ) and (start_time between " + start_day * 1000000 + " and "
							+ end_day * 1000000 + ");";
					pstmt = (PreparedStatement) conn.prepareStatement(sql);
					rs = pstmt.executeQuery();
					rs.next();
